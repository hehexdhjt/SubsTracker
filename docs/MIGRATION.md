...
如果你之前部署过本系统的旧版本，本文说明升级流程、自动迁移做了什么、回滚方法。
若是首次部署，可忽略本文，直接看 [README](../README.md) 的"部署"章节即可。
...
部署后第一次访问任意页面，KV 数据会自动从旧结构迁移到当前结构。**老数据不会丢**，旧 `subscriptions` Key 改名为 `subscriptions_v2_backup` 保留 7 天可回滚。
...
| `scheduler_logs_v3` | 把旧 `scheduler_status_history` 合并到结构化日志表 | `sched_log:{iso}` (TTL 30 天) |

完成后写入 `schema_version = "v3"` 标志位（这个字符串是 KV 内部数据兼容性标记，与"产品版本号"无关）。已迁移的 Worker 实例命中内存缓存，后续请求几乎零开销。
...
| 之前 | 现在 |
|------|------|
| 按 **UTC 小时**解释。例：北京时间 08:00 = UTC 00:00，需填 `00` | 按 **配置的 TIMEZONE** 解释。例：`TIMEZONE=Asia/Shanghai` 时，想北京 08:00 直接填 `08` |

**升级后请到配置页根据实时预览重新调整**——配置页底部会显示"当前你所在时区是 17:30 ✓ 在通知时段内"。

### 2. 默认时区从 `UTC` 改为 `Asia/Shanghai`

新部署的实例，`config.TIMEZONE` 默认就是 `Asia/Shanghai`。已存在的部署，原值会保留——如果你之前是 `UTC` 又是中国用户，建议手动改成 `Asia/Shanghai`。
...
### 4. 旧 `reminderUnit/reminderValue` 字段保留兼容

调用方仍可发旧的单点提醒字段（POST /api/subscriptions），后端在写入时会同时建一条等价规则；列表查询返回的对象仍包含这两个字段。

### 5. 调度状态读取位置变了

| 数据 | 旧 KV Key | 新 KV Key |
|------|-----------|-----------|
...
### 方案 A：版本回滚（如果新版本部署有问题）

```bash
git checkout master  # 或 legacy-v1
npm install
npm run deploy:safe
```

注意：新版本已经写入了新 KV Key 结构，回到旧版本代码后旧版本会读取旧 `subscriptions` Key。在 7 天 TTL 内仍能读到原数据（因为旧 Key 改名为 `subscriptions_v2_backup`，回滚需要手动改回）：
...
- [ ] 登录后台，确认订阅列表完整（数量与升级前一致）
- [ ] 访问 `/debug`，看"时区诊断"区块——`TIMEZONE` 显示的是不是你期望的时区？
- [ ] 访问配置页，调整 `NOTIFICATION_HOURS`（旧的 UTC 小时已无效）
...
**Q：升级后老用户还能继续工作吗？**
A：能。所有原有 API 路径、响应结构、配置字段都保留；只是新增了能力。
...
**Q：能不迁移吗？**
A：不能。当前代码假设新 KV 结构。如果坚持用老版本，请保留 `master` 分支不要 `git pull`。
